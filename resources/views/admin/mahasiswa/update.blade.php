    ...
    <section class="container-dalam max-w-5xl mx-auto bg-surface rounded-2xl shadow-sm border border-border p-6 md:p-10">
        ...
        <div class="mb-8 pb-4 border-b border-border flex justify-between items-end">
            <div>
                ...
                <p class="text-sm text-text-light mt-1">Perbarui informasi akun, data diri, atau status magang mahasiswa.</p>
            </div>
            <!-- Badge Status Saat Ini (Opsional untuk visualisasi cepat) -->
            <div>
                <span class="px-4 py-1.5 bg-background border border-border text-text-light text-sm font-semibold rounded-full">
                    ID: {{ $dataUser->id }}
                </span>
            </div>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-background p-5 rounded-xl border border-border">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" placeholder="••••••••"
                            class="w-full px-4 py-2.5 bg-surface border border-border rounded-lg text-text focus:outline-none focus:border-primary transition-colors">
                        <p class="text-xs text-text-light mt-1.5 italic">* Kosongkan jika tidak ingin mengubah password.</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-background p-5 rounded-xl border border-border">
                    <div>
                        ...
                        <input type="text" name="nim" value="{{ old('nim', $dataUser->mahasiswaProfile->nim ?? '') }}"
                            class="w-full px-4 py-2.5 bg-surface border border-border rounded-lg text-text focus:outline-none focus:border-primary transition-colors">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-text-light mb-1.5">Instansi Asal (Kampus/Sekolah)</label>
                        <input type="text" name="instansi_asal" value="{{ old('instansi_asal', $dataUser->mahasiswaProfile->instansi_asal ?? '') }}"
                            class="w-full px-4 py-2.5 bg-surface border border-border rounded-lg text-text focus:outline-none focus:border-primary transition-colors">
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-background p-5 rounded-xl border border-border">
                    <div>
                        ...
                        <select name="periode_magang_id"
                            ...
                            @foreach ($periodeList as $periode)
                                <option value="{{ $periode->id }}" {{ $periodeLama == $periode->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="dibatalkan" {{ $statusLama == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-text-light mb-1.5">Tanggal Mulai</label>
                        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', optional($dataUser->mahasiswaProfile->tanggal_mulai ?? null)->format('Y-m-d')) }}"
                            class="w-full px-4 py-2.5 bg-surface border border-border rounded-lg text-text focus:outline-none focus:border-primary transition-colors">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-text-light mb-1.5">Tanggal Selesai</label>
                        <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', optional($dataUser->mahasiswaProfile->tanggal_selesai ?? null)->format('Y-m-d')) }}"
                            class="w-full px-4 py-2.5 bg-surface border border-border rounded-lg text-text focus:outline-none focus:border-primary transition-colors">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    @foreach ($skillList as $skill)
                        <label for="skill-{{ $skill->id }}" class="flex items-center gap-2 px-4 py-2 bg-background border border-border rounded-full cursor-pointer hover:border-primary transition-colors">
