<div class="font-montserrat">
    ...
    @else
        {{-- ========================================================= --}}
        {{-- 1. TAMPILAN DESKTOP & TABLET (Tabel Menunggu Review)     --}}
        {{-- ========================================================= --}}
                ...
                <tbody class="divide-y divide-border">
                    @foreach ($tugasMenungguReview as $tugas)
                        <tr class="hover:bg-black/[0.02] transition-colors duration-200 group">
                            ...
                            <!-- Dikumpulkan Oleh (Nama & Info Tim/Individu) -->
                            <td class="px-5 py-4 align-middle">
                                ...
                                @else
                                    <p class="text-[11px] text-text-light mt-0.5">Mengerjakan secara individu</p>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-4 align-middle whitespace-nowrap">
                                <p class="text-xs text-text-light flex items-center gap-1.5">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 shrink-0 text-text-light"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <span class="font-medium text-text">
                                        {{ $tugas->submissions->first()?->created_at?->translatedFormat('d M Y, H:i') ?? '-' }}
                                    </span>
                                </p>
                            </td>
                            ...
                            <!-- Button Periksa Tugas -->
                            <td class="px-5 py-4 align-middle text-center whitespace-nowrap">
                                <x-buttonv2 href="{{ route('asn-submission-detail', $tugas->id) }}" color="accent-dark"
                                    class="text-xs py-1.5 px-3 inline-flex items-center gap-1.5">
        ...
        <div class="grid grid-cols-1 gap-4 md:hidden">
            @foreach ($tugasMenungguReview as $tugas)
                <div
                    ...
                    <!-- Header Mobile: No & Lampiran Badge -->
                    <div class="flex items-center justify-between border-b border-border/60 pb-2.5">
                        ...
                        @if ($tugas->submissions->first()?->file_path)
                            <span
                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/10 text-primary-dark text-[10px] font-bold rounded uppercase tracking-wider border border-primary/20">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                        ...
                        @else
                            <span
                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-background text-text-light text-[10px] font-bold rounded uppercase tracking-wider border border-border">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <!-- Info Mahasiswa & Waktu Pengumpulkan -->
                    <div class="bg-background p-3 rounded-md border border-border/50 space-y-2">
                    ...
                    <!-- Footer Mobile: Button Periksa Tugas -->
