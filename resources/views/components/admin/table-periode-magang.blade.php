<!-- Alert Sukses -->
@if (session('success'))
    <div class="mb-6 p-4 bg-success/10 border border-success/30 rounded-xl flex items-center gap-3 shadow-sm font-montserrat">
...
<div class="bg-surface rounded-xl shadow-sm border border-border overflow-hidden font-montserrat w-full mb-8">
    ...
    <div class="p-5 md:p-6 border-b border-border bg-background flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-[11px] md:text-xs text-text-light mt-1">Kelola gelombang dan waktu pelaksanaan magang di lingkungan BPS.</p>
        </div>
    ...
    <div class="w-full">
        <table class="w-full text-left border-collapse">
            ...
            <thead class="hidden md:table-header-group bg-background/50 border-b border-border text-text-light text-xs uppercase tracking-wider">
                <tr>
            ...
            <tbody class="block md:table-row-group divide-y divide-border md:divide-y-0">
                @forelse ($periodeMagang as $periode)
                    ...
                    <tr class="block md:table-row flex-col p-4 md:p-0 hover:bg-primary/5 transition-colors duration-200 group md:border-b md:border-border last:border-b-0">
                        ...
                        <td class="block md:table-cell md:px-6 md:py-4 mb-3 md:mb-0 border-b border-border border-dashed md:border-none pb-3 md:pb-0">
                            <div class="flex justify-between items-start md:block">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-text group-hover:text-primary transition-colors text-right md:text-left">{{ $periode->nama_periode }}</p>
                                    @if ($periode->keterangan)
                                        <p class="text-[11px] text-text-light mt-0.5 md:line-clamp-1 text-right md:text-left" title="{{ $periode->keterangan }}">
                                            {{ $periode->keterangan }}
                                        </p>
                                    @endif
                                </div>
                        ...
                        <td class="block md:table-cell md:px-6 md:py-4 md:text-center mb-3 md:mb-0">
                            <div class="flex justify-between items-center md:block md:mx-auto">
                                ...
                                <span class="text-sm font-bold text-text">
                                    {{ $periode->kuota }} <span class="text-[10px] md:text-xs text-text-light font-medium">org</span>
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="block md:table-cell md:px-6 md:py-4 md:text-center mb-4 md:mb-0">
                            <div class="flex justify-between md:justify-center items-center">
                                ...
                                @if ($periode->status === 'berlangsung')
                                    <span class="inline-flex px-2.5 py-1 bg-success/10 text-success-dark text-[10px] font-bold rounded-md uppercase tracking-wider border border-success/20 items-center gap-1.5 shadow-sm">
                                        <span class="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></span>
                                        Berlangsung
                                    </span>
                                @elseif ($periode->status === 'akan_datang')
                                    <span class="inline-flex px-2.5 py-1 bg-primary/10 text-primary-dark text-[10px] font-bold rounded-md uppercase tracking-wider border border-primary/20 items-center gap-1.5 shadow-sm">
                                        <span class="w-1.5 h-1.5 rounded-full bg-primary"></span>
                                        Akan Datang
                                    </span>
                                @elseif ($periode->status === 'selesai')
                                    <span class="inline-flex px-2.5 py-1 bg-background text-text-light text-[10px] font-bold rounded-md uppercase tracking-wider border border-border items-center gap-1.5 shadow-sm">
                                        <span class="w-1.5 h-1.5 rounded-full bg-text-light"></span>
                                        Selesai
                                    </span>
                                @else
                                    <span class="inline-flex px-2.5 py-1 bg-background text-text-light text-[10px] font-bold rounded-md uppercase tracking-wider border border-border items-center gap-1.5">
                                        {{ $periode->status }}
                                    </span>
                                @endif
                            </div>
                    ...
                    <tr class="block md:table-row">
                        <td colspan="6" class="block md:table-cell px-6 py-12 md:py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-[11px] md:text-sm text-text-light mt-1">Silakan tambahkan periode baru terlebih dahulu.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
