            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-surface rounded-xl shadow-sm border border-border overflow-hidden">
                    ...
                    <div class="p-5 space-y-4">
                        <div>
                            <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-1">Judul Tugas</p>
                            <p class="text-sm font-bold text-text">{{ $tugas->judul }}</p>
                        </div>
                        <div>
                            <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-1">Tenggat Waktu</p>
                        ...
                        <div>
                            <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-1">Deskripsi</p>
                            <p class="text-xs text-text leading-relaxed whitespace-pre-wrap bg-background p-3 rounded-lg border border-border">{{ $tugas->deskripsi }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-surface rounded-xl shadow-sm border border-border overflow-hidden">
                    ...
                    <div class="p-5">
                        <!-- Ketua -->
                        <div class="mb-3">
                            <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-1">Ketua Tim / Penanggung Jawab</p>
                            <div class="flex items-center gap-2">
                                <span class="w-6 h-6 rounded-full bg-primary/20 text-primary-dark flex items-center justify-center text-xs font-bold">K</span>
                                <p class="text-sm font-medium text-text">{{ $tugas->mahasiswaProfile->user->name ?? '-' }}</p>
                            </div>
                        </div>
                        ...
                        @if ($tugas->anggotaDiterima->isNotEmpty())
                            <div>
                                <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-2">Anggota Tim</p>
                                <ul class="space-y-2">
                                    @foreach ($tugas->anggotaDiterima as $anggota)
                                        <li class="flex items-center gap-2">
                                            <span class="w-6 h-6 rounded-full bg-background border border-border text-text-light flex items-center justify-center text-xs font-bold">A</span>
                                            <p class="text-sm text-text">{{ $anggota->mahasiswaProfile->user->name }}</p>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @if ($tugas->submissions->isEmpty())
                    <div class="py-12 bg-surface border border-border rounded-xl text-center shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-border mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                ...
                <div class="space-y-6">
                    @foreach ($tugas->submissions as $submission)
                        <div class="bg-surface border {{ $loop->first ? 'border-primary' : 'border-border' }} rounded-xl shadow-sm overflow-hidden relative">
                            ...
                            <div class="p-5 md:p-6 space-y-5">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div>
                                        <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-0.5">Dikirim Pada</p>
                                        <p class="text-sm font-bold text-text">{{ \Carbon\Carbon::parse($submission->created_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                    </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <!-- Lampiran File -->
                                    <div>
                                        <p class="text-[11px] font-semibold text-text-light uppercase tracking-wider mb-2">Lampiran File</p>
                                        @if ($submission->file_path)
                                            <a href="{{ Storage::url($submission->file_path) }}" target="_blank"
                                                ...
                                                <span class="text-sm font-medium text-text group-hover:text-primary transition-colors line-clamp-1">
                                    ...
                                    @if ($submission->catatan_mahasiswa)
                                        <div>
                                            ...
                                            <div class="text-sm text-text leading-relaxed bg-[#F8FAFC] border border-border p-4 rounded-lg whitespace-pre-wrap">{{ $submission->catatan_mahasiswa }}</div>
                                        </div>
                                    @endif
                                    ...
                                    @if ($submission->catatan_asn)
                                        <div>
                                            ...
                                            <div class="text-sm text-text leading-relaxed bg-primary/5 border border-primary/20 p-4 rounded-lg whitespace-pre-wrap">{{ $submission->catatan_asn }}</div>
                                        </div>
                                    @endif
                                ...
                                <div class="bg-primary/5 border-t border-primary/20 p-5 md:p-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div class="bg-surface border border-border p-4 rounded-xl">
                                            <form method="POST" action="{{ route('asn-submission-revisi', $submission->id) }}" class="m-0 space-y-3">
                                                @csrf
                                                <label class="block text-xs font-semibold text-text-light uppercase tracking-wider">Minta Revisi Tugas</label>
                                        ...
                                        <div class="bg-surface border border-border p-4 rounded-xl flex flex-col justify-between">
                                            <div>
                                                <label class="block text-xs font-semibold text-text-light uppercase tracking-wider mb-2">Terima Tugas</label>
                                                <p class="text-xs text-text-light leading-relaxed">Jika hasil pekerjaan sudah sesuai standar dan tidak ada yang perlu direvisi, Anda dapat menyetujui tugas ini. Status tugas akan berubah menjadi Selesai.</p>
                                            </div>
                                            <form method="POST" action="{{ route('asn-submission-approve', $submission->id) }}" class="m-0 mt-3" onsubmit="return confirm('Anda yakin ingin menyetujui tugas ini?');">
                                                @csrf
                                                <x-main-button type="submit"
