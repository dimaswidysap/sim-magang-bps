    ...
    <section class="max-w-5xl mx-auto">
        ...
        @if ($errors->any())
            <div class="mb-6 bg-danger/10 border border-danger p-4 rounded-xl flex items-start gap-3">
                <svg class="h-5 w-5 text-danger shrink-0 mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <form method="POST" action="{{ route('asn-update-tugas', $tugas->id) }}" data-confirm="Apakah anda yakin ingin menerapkan perubahan?">
                ...
                <div class="mb-8">
                    ...
                    <div class="bg-[#F8FAFC] border border-border rounded-xl p-5 sm:p-6 space-y-5">
                        ...
                        <!-- Batas Waktu -->
                        <div>
                            <label class="block text-sm font-medium text-text-light mb-1.5">Batas Waktu (Deadline)</label>
                            <input type="datetime-local" name="deadline" value="{{ old('deadline', date('Y-m-d\TH:i', strtotime($tugas->deadline))) }}"
