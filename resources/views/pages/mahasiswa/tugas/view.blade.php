        ...
        <div class="bg-surface rounded-[10px] shadow-sm border border-border overflow-hidden">
            ...
            <div class="p-6 md:p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-background border border-border rounded-xl">
                    <!-- Deadline -->
                    <div>
                        <p class="text-xs font-semibold text-text-light uppercase tracking-wider mb-1.5">Tenggat Waktu (Deadline)</p>
                    ...
                    <!-- Pembuat (ASN) -->
                    <div>
                        <p class="text-xs font-semibold text-text-light uppercase tracking-wider mb-1.5">Pemberi Tugas (ASN)</p>
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-primary-light/20 text-primary-dark flex items-center justify-center font-bold text-sm shrink-0 border border-primary/20">
                ...
                <!-- Deskripsi Tugas -->
                <div>
                    <h2 class="text-sm font-semibold text-text-light uppercase tracking-wider mb-3">Deskripsi / Detail Pekerjaan</h2>
                    <!-- PERBAIKAN: Baris di bawah ini digabungkan menjadi satu agar tidak ada spasi tak diundang di awal paragraf -->
                    <div class="text-sm text-text leading-relaxed bg-surface border border-border p-5 rounded-xl whitespace-pre-wrap">{{ $detailTugas->deskripsi }}</div>
                </div>
            ...
            <div class="p-6 md:px-8 md:py-6 bg-background border-t border-border flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <!-- Tombol Ambil Tugas (Dalam Form) -->
                <!-- Note: Silakan ganti route-nya dengan route untuk mengambil tugas yang sesuai dengan sistem Anda -->
                {{-- action="{{ route('mahasiswa-ambil-tugas', $detailTugas->id) }}" --}}
                <form  method="POST" class="w-full sm:w-auto m-0" onsubmit="return confirm('Apakah Anda yakin ingin mengambil tugas ini?');">
                    @csrf
                    <!-- Tambahkan @method('PUT') atau @method('POST') sesuai dengan logic controller Anda -->
