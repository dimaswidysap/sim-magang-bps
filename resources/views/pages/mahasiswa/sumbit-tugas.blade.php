...
@section('content')
        ...
        <section class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="bg-surface rounded-xl shadow-sm border border-border overflow-hidden">
                ...
                <form method="POST" action="{{ route('mahasiswa-tugas-submit', $tugas->id) }}"
                    data-confirm="Apakah anda yakin ingin mengirimkan tugas?" enctype="multipart/form-data" class="m-0">
                    @csrf
                    <div class="p-6 md:p-8 space-y-6">
                        ...
                        <!-- Input File (Multiple + Preview Area) -->
                        <div>
                            <label class="block text-sm font-semibold text-text-light uppercase tracking-wider mb-2">
                                File Hasil Kerja <span class="normal-case font-normal text-[11px]">(Opsional, bisa memilih lebih dari 1 file)</span>
                            </label>
                            ...
                            <div id="file-preview-container" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4 hidden"></div>
                        </div>
    ...
    <!-- JavaScript untuk Menangani Multiple Preview & Menghapus File -->
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderPreviews() {
                ...
                Array.from(dt.files).forEach((file, index) => {
                    ...
                    card.className = 'relative bg-surface border border-border rounded-xl p-3 flex flex-col items-center text-center shadow-sm';

                    // Tombol Hapus (X) di Pojok Kanan Atas Card Preview
                    const removeBtn = document.createElement('button');
                    removeBtn.type = 'button';
                    removeBtn.className = 'absolute -top-2 -right-2 bg-danger text-white rounded-full p-1 shadow hover:scale-110 transition-transform focus:outline-none';
                    removeBtn.innerHTML = `
                    ...
                    // Cek Tipe File (Gambar / Non-Gambar)
                    let mediaElement = '';
                    if (file.type.startsWith('image/')) {
                        const imgUrl = URL.createObjectURL(file);
                        mediaElement = `<img src="${imgUrl}" class="w-full h-20 object-cover rounded-lg mb-2">`;
                    } else {
                        mediaElement = `
                            <div class="w-full h-20 rounded-lg bg-primary/10 text-primary flex items-center justify-center mb-2 shrink-0">
                    ...
                    card.innerHTML = `
                        ${mediaElement}
                        <p class="text-xs font-semibold text-text truncate w-full" title="${file.name}">${file.name}</p>
                        <span class="text-[10px] text-text-light mt-0.5">${fileSize}</span>
                    `;
