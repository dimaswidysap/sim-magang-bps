@extends('layouts.app')
@include('pages.mahasiswa.logbook.lihat-lampiran')
@section('content')
    <main class="w-full p-4 md:p-8 bg-background min-h-screen font-montserrat">
        <section class="max-w-5xl mx-auto space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
                ...
                <x-buttonv2 href="{{ route('mahasiswa-index') }}" color="accent-dark" class="w-full sm:w-auto">
                                        ...
                                        <tr class="hover:bg-black/[0.02] transition-colors duration-200 group">
                                            ...
                                            <td class="px-5 py-4 align-middle text-center whitespace-nowrap">
                                                <div class="flex items-center justify-center gap-2">
                                                    <x-buttonv2 href="{{ route('logbook-mandiri-edit', $item->id) }}"
                                                        color="accent-dark" class="!px-3 !py-1.5 text-xs">
                                                        Edit
                                                    </x-buttonv2>
                                                    <form method="POST"
                                                        action="{{ route('logbook-mandiri-destroy', $item->id) }}"
                                                        data-confirm="Apakah anda yakin ingin menghapus logbook ini?"
                                                        class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <x-buttonv2 type="submit" color="danger"
                        ...
                        <div class="grid grid-cols-1 gap-4 md:hidden">
                            @foreach ($logbookMandiri as $item)
                                <div class="bg-surface border border-border rounded-[10px] p-4 shadow-sm space-y-3">
                                    ...
                                    <div class="w-full flex pt-3 border-t border-border/60 justify-end gap-2">
                                        <x-buttonv2 href="{{ route('logbook-mandiri-edit', $item->id) }}"
                                            color="accent-dark" class="w-full sm:w-auto text-xs">
                                            Edit
                                        </x-buttonv2>
                                        <form method="POST" action="{{ route('logbook-mandiri-destroy', $item->id) }}"
                                            data-confirm="Apakah anda yakin ingin menghapus logbook ini?"
                                            class="w-full sm:w-auto">
                                            @csrf
                                            @method('DELETE')
                                            <x-buttonv2 type="submit" color="danger" class="w-full sm:w-auto text-xs">
                                                Hapus
                                            </x-buttonv2>
                                        </form>
                                    </div>
