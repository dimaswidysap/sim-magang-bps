...
@section('content')
    <section class="w-full p-3 md:p-6 font-montserrat">
        <div class="max-w-7xl mx-auto">
            ...
            <!-- Header Halaman -->
            <div class="mb-6">
                <h1 class="text-xl md:text-2xl font-bold text-text">Data Magang Pending</h1>
                <p class="text-xs md:text-sm text-text-light mt-1">Daftar pengajuan magang mahasiswa yang sedang menunggu
                    proses verifikasi.</p>
            </div>
            ...
            <div class="mb-6 border-b border-border overflow-x-auto scrollbar-none">
                <nav class="flex space-x-2 md:space-x-4 min-w-max pb-2">
                    ...
                    <a href="{{ route('magang-pending') }}"
                        class="px-4 py-2 text-xs md:text-sm font-semibold rounded-lg bg-amber-500 text-white shadow-sm transition-all">
                        Pending
                    </a>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <thead
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-4 font-bold">Rencana Periode</th>
                                <th class="px-6 py-4 font-bold">Surat Pengantar</th>
                            ...
                            @forelse ($magangPending as $item)
                                @php
                                    ...
                                    $tglSelesai = isset($profile['tanggal_selesai'])
                                        ...
                                        : '-';
                                    $suratPath =
                                        $profile['surat_pengantar_path'] ?? ($profile->surat_pengantar_path ?? null);
                                @endphp

                                <tr class="hover:bg-amber-50/50 transition-colors duration-150 align-middle">
                                    <!-- Nama & NIM -->
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-10 h-10 rounded-full bg-amber-100 text-amber-700 font-bold flex items-center justify-center shrink-0 border border-amber-200 text-sm">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-xs text-text-light">{{ $email }}</div>
                                    </td>

                                    <!-- Surat Pengantar -->
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($suratPath)
                                            <a href="{{ asset('storage/' . $suratPath) }}" target="_blank"
                                        ...
                                        @else
                                            <span class="text-xs text-text-light italic">Belum diunggah</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-100 border border-amber-200 text-amber-700 text-xs font-bold rounded-full">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-text-light">
                                        <p class="text-sm">Tidak ada pengajuan magang yang sedang pending.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
