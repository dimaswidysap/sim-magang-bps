...
class PythonScriptGenerator
{
    ...
    {
        $items = array_map(fn($h) => '    (' . $this->pyString($h['text']) . ", {$h['size']})", $headerWithSizes);
        return "[\n" . implode(",\n", $items) . "\n]";
    }

    private function toPythonMappingList(array $mappings): string
    {
        $items = array_map(fn($m) => '    (' . $this->pyString($m['label']) . ', ' . $this->pyString($m['column']) . ')', $mappings);
        return "[\n" . implode(",\n", $items) . "\n]";
    }
    ...
    private function buildTemplate(string $headerList, string $mappingList, string $matchColumn): string
    {
        ...
        for index, row in df.iterrows():
            ...
            if folder_terpilih:
                ...
                if not files_gambar:
                    # PELANGGARAN: Folder ditemukan tapi kosong
                    pelanggaran_ini.append({
                        'nomor_file': '-',
                        'alasan': 'Folder foto kosong (tidak ada gambar valid)'
                    })
                else:
                ...
                # PELANGGARAN: Folder tidak ditemukan
                print(f"  -> [!] Folder untuk {nilai_cocok} tidak ditemukan.")
                pelanggaran_ini.append({
                    'nomor_file': '-',
                    'alasan': 'Folder foto tidak ditemukan'
                })
        ...
        if daftar_pelanggar:
            ...
            run_title = p_title.add_run("DAFTAR PELANGGARAN / KENDALA FOTO")
            run_title.bold = True
